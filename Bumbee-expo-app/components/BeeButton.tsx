...
export function BeeButton({ title, onPress, loading, disabled, variant = 'primary', style }: Props) {
  const bg = variant === 'facebook' ? Colors.facebook : variant === 'secondary' ? Colors.white : Colors.primary;
  ...
  return (
    <TouchableOpacity
      ...
      style={[styles.button, { backgroundColor: bg, borderColor, borderWidth: variant === 'secondary' ? 1 : 0, opacity: disabled ? 0.5 : 1 }, style]}
      activeOpacity={0.8}
    >
      {loading ? (
